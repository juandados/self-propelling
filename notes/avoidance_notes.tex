...
We will denote $p_{ij}=p_{i}-p_{j}$ and $h_{i}$ will be the shortest
vector connecting $p_{i}$ with $\Gamma$. The proposed control force
is given as

\[
u_{i}=-\stackrel[j\neq i]{n}{\sum}\left(f_{I}\left(\left\Vert p_{ij}\right\Vert \right)\frac{p_{ij}}{\left\Vert p_{ij}\right\Vert }\right)-f_{h}\left(\left\Vert h_{i}\right\Vert \right)\frac{h_{i}}{\left\Vert h_{i}\right\Vert }+f_{v}\left(\left\Vert v_{i}\right\Vert \right)\frac{v_{i}}{\left\Vert v_{i}\right\Vert }
\]
...
\begin{align*}
\phi\left(z\right) & =\frac{-2\left(p_{x}v_{x}+p_{y}v_{y}\right)-\sqrt{4\left(p_{x}v_{x}+p_{y}v_{y}\right)^{2}-4\left(v_{x}^{2}+v_{y}^{2}\right)\left(p_{x}^{2}+p_{y}^{2}-c_{r}^{2}\right)}}{2\left(v_{x}^{2}+v_{y}^{2}\right)}\\
